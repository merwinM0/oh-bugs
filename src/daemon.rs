...
pub fn run() -> anyhow::Result<()> {
    ...
    'main: loop {
            ...
            match shell.try_recv_output() {
                Ok(data) => {
                    ...
                    if match_count > 0 {
                        ...
                        mgr.trigger(match_count, Duration::from_millis(bug_lifetime), &screen);
                        // 触发后立即保存并绘制
        ...
        // 4. 动画 tick：更新虫子位置 → 恢复旧位置 → 绘制新位置
        {
            let mut mgr = bug_manager.lock().unwrap();
            // 更新：恢复移走的虫子位置
            if let Ok(to_restore) = mgr.update(&mut std::io::stdout(), &mut screen) {
                let _ = mgr.restore_positions(&mut std::io::stdout(), &to_restore);
            }
            // 重新绘制所有虫子
            let _ = mgr.draw_to(&mut std::io::stdout(), &mut screen);
            let _ = std::io::stdout().flush();
