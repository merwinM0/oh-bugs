...
| 数量     | 每次触发随机生成 2~5 只                               |
| 出现位置 | 终端上半区的随机位置                                  |
| 移动方式 | 水平：60% 继续方向 / 30% 停留 / 10% 反向；垂直：远离边缘时 -2~+2 行，近边缘时强制向中心走 |
| 随机种子 | 每 tick 使用递增计数器 × 大质数 + 每虫独立 phase，确保每 tick 每虫轨迹不同 |
| 生命周期 | 每只虫子独立随机 2~3 秒（基于配置中心值 ±500ms），到期自动消失并恢复文字 |
| 最大并发 | 屏幕同时最多显示 30 只虫子                            |
| 边缘处理 | 距边缘 <3 列时强制向中心方向走，避免堆积在屏幕边缘 |
| 消失恢复 | 超时后立即恢复被覆盖的原始字符，不会残留             |
...
PTY master read   ──→  [1] 写入真实终端 stdout
                         ...
                         [4] 送入 Watcher.scan()
                              ↓ 匹配 error
                         [5] BugManager.trigger() 生成虫子 (saved_char=None)
                         [6] BugManager.draw_to() 对新虫子
...
BugManager.update()  — 每只虫子原子操作：
    ├─ 恢复旧位置原始字符 (Terminal.clear_at_to + screen.restore_char)
    ├─ 随机移动（边缘排斥 + 惯性游走）
    ├─ 保存新位置原始字符 (screen.save_char)
    └─ 绘制虫子 Emoji (Terminal.draw_bug_to)
